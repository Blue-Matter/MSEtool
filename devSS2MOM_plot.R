...
fl = 1
om <- C_dat %>% filter(Year>=1993, Sim==1, Fleet==fl, Model=="OM")
ss <- C_dat %>% filter(Year>=1993, Fleet==fl, Model=="SS")

data.frame(om$Year, om$C/ss$C)


plot(multiHist[[1]][[fl]]@SampPars$Fleet$V_real[1,,44], ylim=c(0,1))
lines(multiHist[[1]][[fl]]@SampPars$Fleet$retA_real[1,,44])
...
om <- C_OM %>% filter(Year>1991, Sim==1, Fleet==1)
ss <- C_SS %>% filter(Year>1991, Fleet==1)

data.frame(Year=om$Year, OM=om$C, SS=ss$C, ratio=ss$C/om$C)

rowSums(multiHist[[1]][[1]]@TSdata$Landings[1,,])/rowSums(multiHist[[1]][[1]]@TSdata$Removals[1,,])


yrs <- 44
plot(multiHist[[1]][[1]]@SampPars$Fleet$V[1,,yrs])
lines(multiHist[[1]][[1]]@SampPars$Fleet$retA[1,,yrs])
