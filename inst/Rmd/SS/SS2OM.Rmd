...
SSB_SS <- replist$timeseries %>% filter(Yr %in% OM_years) %>% group_by(Yr) %>% 
  summarise(SpawnBio = mean(SpawnBio, na.rm = TRUE)) %>% getElement("SpawnBio")
...
if(replist$SS_versionNumeric == 3.30) {
  N_at_age <- reshape2::melt(replist$natage[replist$natage$`Beg/Mid` == "B" & replist$natage$Seas == 1, -c(9:12)], 
    ...
    dplyr::filter(Yr %in% OM_years, Sex %in% gender)
} else {
  
  R_main <- replist$timeseries$Yr %in% OM_years
  ...
  N_at_age <- reshape2::melt(replist$natage[replist$natage$`Beg/Mid` == "B" & replist$natage$Seas == 1, -c(8:11)], 
                             ...
                             variable.name = "Age", value.name = "N") %>% 
    dplyr::filter(Yr %in% OM_years, Sex %in% gender) %>% group_by(Yr, Age) %>% summarise(N = sum(N)) %>%
