          ...
          for (int fl = 0; fl < nFleet; ++fl) {
            for (int age = 0; age < nAge; ++age) {
              if (Z_age[age] > 0.0) {
                ...
                LAA_st(sim_laa, age, y, fl, area) = Fr_ratio * N_dead_age[age];
                DAA_st(sim_daa, age, y, fl, area) = Fdisc_ratio * N_dead_age[age];
              } else { 
                LAA_st(sim_laa, age, y, fl, area) = 0.0;
                DAA_st(sim_daa, age, y, fl, area) = 0.0;
              } 
            } // end age loop 
          } // end fleet loop
      } // end area loop
