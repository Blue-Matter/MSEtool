...
#'   validated and dimension names are applied. If `AC` is non-zero and
#'   `Error` was freshly generated (i.e. not already supplied by the user),
#'   the error series is subsequently transformed to an AR(1) process (see
#'   below). A user-supplied `Error` array is never transformed by `AC`,
#'   since it may already encode its own autocorrelation structure.
...
#'
#' When `AC` is non-zero and `Error` was freshly generated, the
#' independently-drawn lognormal error series is transformed into a
#' variance-preserving AR(1) process on the log scale.
