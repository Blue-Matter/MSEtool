...
UpdateRetention <- function(ProjSim, MPAdvice, MPAdvicePrevious, YearsAll, TSIndex) {
  UpdateSelectivity(ProjSim, MPAdvice, MPAdvicePrevious, YearsAll, TSIndex, 'Retention')
}

UpdateDiscardMortality <- function(ProjSim, MPAdvice, MPAdvicePrevious, YearsAll, TSIndex) {
...
UpdateSelectivity <- function(ProjSim, MPAdvice, MPAdvicePrevious, 
                              YearsAll, TSIndex, type=c('Selectivity', 'Retention')) {
  type <- match.arg(type)
  ...
  for (st in 1:nStock) {
    ...
    for (fl in 1:nFleet) {
      ...
      LengthWeight@MeanAtAge <- LengthWeight@MeanAtAge |> ArraySubsetYear(Year)
      LengthWeight@CVatAge <- LengthWeight@CVatAge |> ArraySubsetYear(Year)
      ...
      if (LengthModel) {
        ...
        Selectivity <- MeanAtLength2MeanAtAge(Selectivity, LengthWeight, Ages, nsim=1, Years=Year)
        
      }  else if (WeightModel) {
        Selectivity <- PopulateMeanAtWeight(Selectivity, LengthWeight, Years=Year, Ages, nsim, seed, silent)
        Selectivity@MeanAtWeight <- Selectivity@MeanAtWeight |> ExpandYears(Years=YearProj)
        ...
        # Selectivity <- PopulateMeanAtAge(Selectivity, Ages, Years=Year)
        # Selectivity@MeanAtAge <- Selectivity@MeanAtAge |> ExpandYears(Years=YearProj)
      }
