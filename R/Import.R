...
BAMGetObject <- function(x, type=c('rdat', 'dat')) {
  type <- match.arg(type)
  type <- paste0(type,'_')
  stockName <- gsub(' ', '',x)
...
BAMGetDiscardMortality <- function(x, TimeSteps, nFleet) {
  ...
  DiscMortDF$Year[(nFleet+1):nrow(DiscMortDF)] <- substr(text, start = 1, stop = 4) |> as.numeric()
