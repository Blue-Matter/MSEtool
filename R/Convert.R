...
om2OM <- function(OM) {
  ...
  OMout@Common_Name <- OM@Stock@CommonName
  OMout@Species <- OM@Stock@Species
  OMout@maxage <- OM@Stock@Ages@MaxAge
    
  # TODO - make generic wrapper for om2stock and om2fleet
  
#   "cpars"  
#   
# [13]                 
# [19] "R0"            "M"             "Msd"           "h"             "SRrel"         "Perr"         
# [25] "AC"            "Linf"          "Linfsd"        "K"             "Ksd"           "t0"           
# [31] "LenCV"         "L50"           "L50_95"        "D"             "a"             "b"            
# [37] "Size_area_1"   "Frac_area_1"   "Prob_staying"  "Fdisc"         "nyears"        "CurrentYr"    
# [43] "EffYears"      "EffLower"      "EffUpper"      "Esd"           "qinc"          "qcv"          
# [49] "L5"            "LFS"           "Vmaxlen"       "isRel"         "LR5"           "LFR"          
# [55] "Rmaxlen"       "DR"            "Spat_targ"     "MPA"    
  ...
  for (st in 1:nStock(OM)) {
    ...
    for (fl in 1:nFleet(OM)) {
      ...
      if (fl ==1) {
        ...
        if (!is.null(OM@Stock[[st]]@Spatial@Movement)) 
          warning('`Spatial@Movement` not done yet!')
        Cpars$mov <- NULL
