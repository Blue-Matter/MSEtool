...
calculate_single_fleet_dynamics <- function(x) { 
  ...
  if(length(x) > 1) { 
    ...
    out <- list(Find = Find_out, V = V_avg, retA = retA_avg, Fdisc = Fdisc_avg)
    
  } else {
    out <- lapply(c("Find", "V", "retA", "Fdisc"), function(xx) getElement(x[[1]], xx)) %>%
      structure(names = c("Find", "V", "retA", "Fdisc"))
  }
  return(out)
}