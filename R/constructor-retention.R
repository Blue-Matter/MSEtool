...
#'   dimensions `Sim x Class x Year`. If provided and `Pars` is empty, takes
#'   precedence over `MeanAtAge`; `MeanAtAge` is derived from it via the
#'   age-length key. A numeric vector is also accepted and converted to a
...
#'   dimensions `Sim x Class x Year`. A numeric vector is also accepted and
#'   converted the same way as `MeanAtLength`, using the [weight-class]
...
#'   retention schedule. Set `FALSE` when retention is only meaningfully
