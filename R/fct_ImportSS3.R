...
ImportSS <- function(SSDir,
  ...
  if (!is.null(DotsList$UpdateRecDevs)) {
    OM <- UpdateRecDevs(replist = RepList[[1]], OM)
  }

  if (!is.null(DotsList$populate)) {
    return(OM)
  }
...
GetSS_Fecundity_seasonal <- function(st, replist, YearsList, Ages) {
  ...
  fec_age <- fec_age |> dplyr::filter(Age %in% Ages@Classes)
...
GetSS_SelectivityAtAge <- function(st, fl, replist, YearsList, Stock) {
  ...
  AgeClasses <- Ages@Classes
  SSAgeClasses <- GetSSAgeClasses(replist)
  AgeSelect <- dplyr::filter(
    replist$ageselex, Fleet == fl, Sex %in% st,
    Factor == "Asel2",
...
UpdateRecDevs <- function(replist, OM) {
  # This is required for the seasonal NPSWO model
  # The plus-group calculations in seasonal SS3 models are different than expected

  # This function calculates the correct rec devs

  Hist <- Simulate(OM, nSim = 1, Reference = FALSE, ConditionObs = FALSE, GenerateData = FALSE, silent = TRUE)

  SSAgeClasses <- GetSSAgeClasses(replist)

  birthseas <- GetSSBirthSeas(replist)
  stnames <- StockNames(OM)

  SS_Recruits <- replist$natage |>
    dplyr::filter(
      Yr >= replist$startyr,
      Yr <= replist$endyr,
      `Beg/Mid` == "B"
    ...
    tidyr::pivot_longer(
      cols = as.character(SSAgeClasses), names_to = "Age",
      values_to = "Value"
    ) |>
    dplyr::select(Yr, Stock = Sex, Seas, Time, Age, Value) |>
    dplyr::filter(Age == 0, Seas == birthseas)

  SS_Recruits$Stock <- dplyr::case_match(
    SS_Recruits$Stock,
    1 ~ stnames[1],
    2 ~ stnames[2],
  )

  SS_Recruits <- SS_Recruits |>
    dplyr::select(Year = Yr, Stock, Value) |>
    dplyr::mutate(Model = "SS3")

  OM_Recruits <- Number(Hist, byAge = TRUE) |>
    dplyr::filter(Age == min(Age)) |>
    dplyr::mutate(Value = ifelse(Value <= 1E-6, 0, Value))

  OM_Recruits <- OM_Recruits[seq(birthseas, by = Seasons(OM), to = nrow(OM_Recruits)), ]

  OM_Recruits <- OM_Recruits |>
    dplyr::select(Year, Stock, Value) |>
    dplyr::mutate(Model = "OM")

  OM_Recruits$Dev <- SS_Recruits$Value / OM_Recruits$Value

  # Update Rec Devs
  for (st in seq_along(stnames)) {
    df <- OM_Recruits |> dplyr::filter(Stock == stnames[st])
    currdev <- OM@Stock[[st]]@SRR@RecDevHist
    newdev <- rep(df$Dev, each = OM@Seasons)
    newdev <- matrix(newdev,
      nrow = nrow(currdev),
      ncol = length(newdev)
    )
