...
#'
#' Import a completed Stock Synthesis 3 (SS3) model and convert it
#' into an \code{\link{OM}} object.
#'
#' @details
#' \code{ImportSS()} reads SS3 output files from \code{SSDir} and constructs an
#' operating model containing stock, fleet, data, observation, and allocation
#' components. Biological processes, fleet structure, and historical time series
#' are inferred directly from the SS3 report output and mapped into the
#' corresponding OM slots.
#'
#' Multiple simulation replicates are supported when multiple SS3 runs are
#' detected. Optional metadata describing authorship, region, and stock identity
#' are stored in the resulting OM. By default, the OM is fully populated using
#' \code{PopulateOM()}, but this step can be skipped using `Populate=FALSE`.
#'
#' \code{ImportSSReport()} reads SS3 output generated by Stock Synthesis 3, either from
#' one or more directories or from existing \code{r4ss} report objects, and
#' returns a standardized list of SS3 report lists.
#'
...
#'   multiple SS3 runs are detected as `nSim` will be set to the number of
#'   SS3 runs that are imported. 
...
#' @param FleetNames Optional character vector of fleet names. If \code{NULL},
...
#' @param silent Logical; if \code{TRUE}, suppress informational output during
#'   import.
#' @param Populate Logical; if \code{TRUE} (default), populate the OM using
#'   \code{PopulateOM()}. If \code{FALSE}, return the partially constructed OM.
#' @param ... Additional arguments passed to SS3 import functions.
#'
#' @return
#' \code{ImportSS()} returns an object of class \code{\link{OM}} containing stock,
#' fleet, data, observation, and allocation components derived from the SS3 model.
#'
#' \code{ImportSSReport()} returns a list of SS3 report objects.
#'
#' @section Functions:
#' \describe{
#'   \item{\code{ImportSS()}}{Convert SS3 output into a fully specified operating
#'   model suitable for management strategy evaluation.}
#'   \item{\code{ImportSSReport()}}{Read SS3 output from directories or existing
#'   \code{r4ss} report objects and return a standardized list of SS3 reports.}
#' }
#'
#' @seealso
#' \code{\link{OM}}, \code{\link{PopulateOM}},
#' \href{https://nmfs-ost.github.io/ss3-website/}{Stock Synthesis 3}
#'
#' @export
#'
#' @examples
...
GetSS_SelectivityAtAge <- function(st, fl, replist, YearsList, Stock) {
  ...
  if (replist$nseasons == 1) {
    ...
    # normalize by year
    return(
      sweep(AgeSelect, 2, apply(AgeSelect, 2, max), "/")
    )
  }

  # Seasonal Model
  nAge <- length(AgeClasses)
  nYear <- length(YearsHist)
  nSeas <- YearsList$Seasons

  MeanAtAge <- matrix(NA_real_,
    nrow = nAge,
    ncol = nYear,
    dimnames = list(
      Age = AgeClasses,
  ...
  AgeSel_long <- AgeSel_df |>
    tidyr::pivot_longer(dplyr::all_of(SSAgeClasses),
      names_to = "Age",
      values_to = "V"
    ) |>
    dplyr::mutate(Age = as.numeric(Age))

  # Precompute seasonal age grid (same every year)
  minAge <- min(AgeSel_long$Age)
  maxAge <- max(AgeSel_long$Age)

  SeasonalAges <- seq(minAge, maxAge + (nSeas - 1) / nSeas, by = 1 / nSeas)

  # Indices map seasonal to integer age
  age_ind <- match(SeasonalAges,
    AgeClasses,
    nomatch = 0
  )
  ...
  YearsAll <- c(YearsList$YearsHist, YearsList$YearsProj)
  for (yr in unique(AgeSel_long$Yr)) {
    tmp <- AgeSel_long[AgeSel_long$Yr == yr, ]
    tmp <- tmp[order(tmp$Age, tmp$Seas), ]

    # Assign seasonal ages
    tmp$Age <- SeasonalAges[seq_len(nrow(tmp))]
    tmp <- tmp[valid, ]

    ind1 <- match(yr, YearsAll)
    ind2 <- match(yr + 1, YearsAll) - 1

    MeanAtAge[age_ind, ind1:ind2] <- tmp$V
  }

  # normalize by year
  sweep(MeanAtAge, 2, apply(MeanAtAge, 2, max), "/")
}
