...
#' @param slot_name `"Landings"` (default) or `"Discards"`.
...
#' @export
CheckCatch <- function(Data, slot_name = 'Landings', incUnits = TRUE) {
  obj <- slot(Data, slot_name)
  ...
  if (is.null(obj@Value)) {
    cli::cli_alert_warning(
      'No {.val {slot_name}} data available for: {.val {Data@Name}}'
      )
    cli::cli_alert_warning(
  ...
  if (!is.null(obj@Units) && any(obj@Units !='Biomass') && incUnits) {
    cli::cli_alert_warning(
      'Caution: TAC is always set in units of {.val Biomass} but some \\
      {.val {slot_name}} values are not units of {.val Biomass}'
      )
    cli::cli_alert_warning(
      'Using these {.val {slot_name}} data may result in incorrect TAC values'
      )
  } 
