PopulateSpatial <- function(Spatial,
  ...
  if (!is.null(Spatial@Movement)) {
    # Movement matrix has been provided 
    # Calculate Unfished Distribution and do checks
  
    # Check Spatial@Movement has correct structure and named dimensions
  ...
  # unique years
  Years_actual <- dnames[["Year"]] |> as.numeric()
  ...
  Grid <- expand.grid(sim=1:nsim, age=1:nAge, year=1:nYear)
