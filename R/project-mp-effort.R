...
Distribute_Effort_Area <- function(Proj,
  ...
  if (!is.array(Advice@Effort))      return(no_change)
  if (length(dim(Advice@Effort)) == 1) return(no_change)
  ...
  Distribution <- abind::adrop(Proj@Distribution[sim, ProjInd, , , drop = FALSE], 1:2) * arr
