...
List CalcPopDynamics2_(Rcpp::List OMListSim,
  ...
  for (int timestep=0; timestep<nTS; timestep++) {
    ...
    for (int st=0; st<nStock; st++) {
      ...
      Biomass[TSindex] = arma::accu(WeightAtAge.col(TSindex) % arma::sum(NumberAtAgeAreaThisTS,1));
     ...
     FDeadAtAgeAreaStock = FDeadAtAgeAreaList[st];
     FRetainAtAgeAreaStock = FRetainAtAgeAreaStock[st];
     ...
     CalcFArea_(FDeadAtAgeAreaThisTS, FRetainAtAgeAreaThisTS,
                RelativeSizeList[st], CatchabilityList[st]);
